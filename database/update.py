...
class Update:
    ...
    def __init__(self,table,fields,conditions=()):
        fields=Update.validate_fields(fields)
        conditions=Update.validate_conditions(conditions)
    ...
    def to_sql(self):
        ...
        for index,field in enumerate(self.fields):
            if index==0:
                fields=fields+field+'="{value}"'.format(value=self.fields[field])
            else:
                fields=fields+','+field+'="{value}"'.format(value=self.fields[field])
        ...
        if conditions=='':
            return 'update {table} set {fields} '.format(fields=fields,table=self.table)
        else:
            return 'update {table} set {fields} where {conditions}'.format(fields=fields,table=self.table,conditions=conditions)